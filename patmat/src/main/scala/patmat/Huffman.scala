...
object Huffman {
  ...
  freqs sortWith (_._2 < _._2) map (leaf => new Leaf(leaf._1, leaf._2))
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
    def helper(x: Bit)(entry: (Char, List[Bit])): (Char, List[Bit]) = entry match {
      case (char, list) => (char, x :: list)
    }
